    ...
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">
          Hello, {patientProfile?.full_name?.split(' ')[0] ?? 'there'} 👋
        </h1>
      ...
      {pendingRequests.length > 0 && (
        <div className="mb-6 rounded-xl bg-amber-50 border border-amber-200 p-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-700 mt-0.5">A hospital is requesting access to your records</p>
          </div>
          <Link to="/patient/access-requests" className="text-xs font-medium text-amber-800 hover:underline">
            Review →
          </Link>
        </div>
      )}
